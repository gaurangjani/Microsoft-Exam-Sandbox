  ...
  try {
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        throw new Error(
          `Model "${model}" did not respond within 8 seconds. Free-tier (":free") models on OpenRouter are often overloaded and slow. Switch OPENROUTER_MODEL in Vercel to a paid model like "openai/gpt-3.5-turbo" or "openai/gpt-4o-mini".`
        );
      }
      throw fetchError;
    } finally {
...
function buildPrompt(examOutline, numQuestions, batchIndex = 0) {
  ...
  return `Generate ${numQuestions} Microsoft exam practice questions for ${examOutline.code}.

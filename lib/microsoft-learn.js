 ...
 * Format: https://learn.microsoft.com/en-us/certifications/EXAM-CODE
 */
const EXAM_URLS = {
 ...
 * Looks for "Skills measured" section
 */
function parseExamPage(examCode, html) {
  const exam = EXAMS.find(e => e.code === examCode);
  if (!exam) return null;

  // Look for skills section - Microsoft Learn uses various formats
  // Pattern 1: <h2>Skills measured</h2> followed by content
  const skillsMatch = html.match(/skills[^<]*measured[^<]*<\/[^>]*>[\s\S]*?(?=<h2|<h3|$)/i);

  if (!skillsMatch) {
    console.warn(`Could not parse skills for ${examCode}`);
    return null;
  }

  const skillsSection = skillsMatch[0];

  // Extract individual skills - look for patterns like:
  // - Skill name (XX%) or
  // - **Skill name** or
  // - Skill name followed by list items
  const skills = [];

  // Pattern for bullet points with percentage
  const skillPattern = /(?:[-•*]|<li>)\s*([^<(\n]+?)\s*\(?(\d+)%?\)?\s*(?=<|$)/g;
  let match;

  while ((match = skillPattern.exec(skillsSection)) !== null) {
    const skillName = match[1].trim();
    const weight = match[2] ? parseInt(match[2]) : 25;

    if (skillName.length > 5 && !skillName.toLowerCase().includes('html')) {
      skills.push({
        area: skillName,
        weight: weight,
        topics: [
          ...
          `Troubleshoot issues related to ${skillName.toLowerCase()}`,
        ],
      });
    }
  }

  // If parsing found no skills, use fallback
  if (skills.length === 0) {
    console.warn(`No skills parsed from HTML for ${examCode}, using fallback`);
    return null;
  }
  ...
  if (totalWeight !== 100 && totalWeight > 0) {
    ...
    skills.forEach(s => (s.weight = Math.round(s.weight * scale)));
  }
...
export async function fetchExamCatalog() {
  // Fetches from centralized config (exams-config.js)
  // This allows easy updates: just edit exams-config.js to add new exams!
  // Skills outlines are fetched from Microsoft Learn on-demand
  return EXAMS;
}
...
export async function fetchExamOutline(examCode) {
  ...
  // Strategy: Try to fetch from Microsoft Learn first, fallback to hardcoded or auto-generated
  ...
  // Step 1: Try to fetch from Microsoft Learn documentation (live data)
  ...
  return generateGenericOutline(examCode, exam);
}

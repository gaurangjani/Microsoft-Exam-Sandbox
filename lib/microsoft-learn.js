...
/**
 * Fetches and caches all Microsoft certification exams dynamically
 * Discovers new exams automatically without code changes
 */
async function fetchAllExamsFromMicrosoftLearn() {
  try {
    // Fetch the main certifications page which lists all available exams
    const response = await fetch('https://learn.microsoft.com/en-us/certifications/', {
      headers: {
    ...
    if (!response.ok) {
      console.error('Failed to fetch certifications page:', response.status);
      return null;
    }

    const html = await response.text();
    const exams = parseAllExamsFromPage(html);

    if (exams && exams.length > 0) {
      console.log(`✓ Discovered ${exams.length} Microsoft certifications`);
      return exams;
    }

    return null;
  } catch (error) {
    console.error('Error fetching exams from Microsoft Learn:', error);
    return null;
  }
}
